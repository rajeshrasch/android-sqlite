...
package io.oracle.android.database.benchmark;
...
public class Benchmark {
    ...
    private void testRequerySQLiteWrite(Statistics statistics) {
        ...
        io.oracle.android.database.sqlite.SQLiteDatabase db = requerySQLite.getWritableDatabase();
        io.oracle.android.database.sqlite.SQLiteStatement statement = db.compileStatement(
            String.format("insert into %s (%s, %s) values (?,?)",
                Record.TABLE_NAME,
                Record.COLUMN_CONTENT,
                Record.COLUMN_CREATED_TIME));
    ...
    private void testRequerySQLiteRead(Statistics statistics) {
        ...
        Cursor cursor = null;
        try {
            io.oracle.android.database.sqlite.SQLiteDatabase db =
    ...
    private static class RequerySQLite extends
        io.oracle.android.database.sqlite.SQLiteOpenHelper {
        public RequerySQLite(Context context, String name) {
            super(context, name, null, 1);
        }

        @Override
        public void onCreate(io.oracle.android.database.sqlite.SQLiteDatabase db) {
            db.execSQL(Record.CREATE_STATEMENT);
        }

        @Override
        public void onUpgrade(io.oracle.android.database.sqlite.SQLiteDatabase db,
